...
void print_all_products(Inventory* inv) {
	if (inv->length == 0) {
	...
	for (int i = 0; i < inv->length; i++) {
		printf("%d. ", inv->list[i].id);
		printf("tip: %s, ", inv->list[i].type);
		printf("prod: %s, ", inv->list[i].manufacturer);
		printf("model: %s, ", inv->list[i].model);
		printf("pret: %d, ", inv->list[i].price);
		printf("cant: %d\n", inv->list[i].quantity);
	}
}
...
int add_product_ui(Service* serv) {
	int id;
	char type[30];
...
void update_product_price_ui(Service* serv, int product_id) {
	int new_price = 0;
...
void update_product_quantity_ui(Service* serv, int product_id) {
	int new_quantity = 0;
...
void update_product_ui(Service* serv) {
	int id;
	if (!read_integer(&id, "id: ")) {
		printf("Id invalid!\n");
		return;
...
int delete_product_ui(Service* serv) {
	int id;
	if (!read_integer(&id, "id: ")) {
		printf("Id invalid!\n");
		return 0;
...
void sort_products_ui(Service* serv) {
...
void filter_by_manufacturer_ui(Service* serv) {
	char manufacturer[30];
	while (!read_char_array(manufacturer, 30, "producator: "))
		printf("Producator invalid!\n");

	char err_msg[30];
	Inventory* inv = filter_by_manufacturer_serv(serv, manufacturer, err_msg);
	if (inv->length == 0)
		printf(err_msg);
	else
...
void filter_by_price_ui(Service* serv) {
	int price;
	while (!read_integer(&price, "pret: "))
		printf("Pret invalid!\n");

	char err_msg[30];
	Inventory* inv = filter_by_price_serv(serv, price, err_msg);
	if (inv->length == 0)
		printf(err_msg);
	else
		print_all_products(inv);
...
void filter_by_quantity_ui(Service* serv) {
	int quantity;
	while (!read_integer(&quantity, "cantitate: "))
		printf("Cantitate invalida!\n");

	char err_msg[30];
	Inventory* inv = filter_by_quantity_serv(serv, quantity, err_msg);
	if (inv->length == 0)
		printf(err_msg);
	else
		print_all_products(inv);
...
void filter_products_ui(Service* serv) {
	if (serv->inv->length == 0) {
		printf("Nu exista produse disponibile!\n");
		return;
	}
...
int main() {
	...
	Product prd3 = { 3, "laptop", "asus", "dagthbr", 4000, 5 };
